...
Status test_architecture_profiles(TestPointRunner &runner)
{
    ...
    for (auto architecture : architectures)
    {
        ...
        if (auto status = runner.check(module, "identity", operations.architecture() == architecture,
        ...
        if (auto status =
                runner.check(module, "register-count", operations.register_count() > 0, "invalid register count");
            !status.ok())
        {
            return status;
        }
        if (auto status = runner.check(module, "interrupt-model", !operations.interrupt_model().empty(),
                                       "missing interrupt model");
            !status.ok())
        {
            return status;
        }
        if (auto status =
                runner.check(module, "syscall-model", !operations.syscall_model().empty(), "missing syscall model");
            !status.ok())
        {
            return status;
        }
        if (auto status = runner.check(module, "user-transition-model", !operations.user_transition_model().empty(),
        ...
        if (auto status = runner.check(module, "kernel-context",
        ...
        if (auto status =
                runner.check(module, "user-context",
                             user_context.mode == arch::PrivilegeMode::user && user_context.registers[0] == 0x55,
        ...
        if (auto status = runner.check(module, "cycle-counter", operations.read_cycle_counter() != 0,
                                       "cycle counter returned zero");
            !status.ok())
        {
            return status;
        }
        ...
        operations.halt();
        if (auto status = runner.check(module, "control-ops", true, "architecture control operations failed");
            !status.ok())
        {
            return status;
        }
...
Status test_interrupts(TestPointRunner &runner)
{
    ...
    if (auto status = runner.check_status(
            "interrupt", "register",
            dispatcher.register_callback(90, "debug-test", nullptr,
                                         [](void *, arch::TrapFrame &) { return Status::success(); }));
        !status.ok())
    {
        return status;
    }
...
Status test_memory(TestPointRunner &runner)
{
    ...
    if (auto status =
            runner.check_status("memory", "map", manager.kernel_address_space().map(0x2000, frame.value(), 0b11));
        !status.ok())
    {
        return status;
    }
    if (auto status = runner.check_status("memory", "unmap", manager.kernel_address_space().unmap(0x2000));
        !status.ok())
    {
        return status;
    }
...
Status test_scheduler(TestPointRunner &runner)
{
    ...
    if (auto status = runner.check_status("sched", "runnable-first", scheduler.set_runnable(first.value()));
        !status.ok())
    {
        return status;
    }
    if (auto status = runner.check_status("sched", "runnable-second", scheduler.set_runnable(second.value()));
        !status.ok())
    {
        return status;
    }
    auto selected = scheduler.schedule_next();
    if (auto status = runner.check("sched", "schedule", selected.ok() && selected.value() != 0,
                                   "scheduler did not select a process");
        !status.ok())
    {
        return status;
    }
    auto selected_on_cpu = scheduler.schedule_next_on_cpu(1);
    return runner.check("sched", "schedule-cpu",
...
Status test_smp(TestPointRunner &runner)
{
    ...
    if (auto status = runner.check("smp", "least-busy", least_busy.ok(), "least busy CPU selection failed");
        !status.ok())
    {
        return status;
    }
...
Status test_ipc(TestPointRunner &runner)
{
    ...
    return runner.check("ipc", "receive", message.ok() && message.value().size == sizeof(Payload),
                        "IPC receive failed");
}
...
{
    syscall::Table table;
    if (auto status = runner.check_status("syscall", "register",
                                          table.register_callback(syscall::Number::ok_debug, "debug", nullptr,
                                                                  [](void *, const syscall::Request &request) {
                                                                      return syscall::Response{
                                                                          .value = static_cast<i64>(request.args[0]),
...
Status test_drivers(TestPointRunner &runner)
{
    ...
    return runner.check("driver", "find",
                        manager.find(driver::Class::timer) != nullptr &&
                            manager.find(driver::Class::display) != nullptr,
...
Status test_filesystem(TestPointRunner &runner)
{
    ...
    if (auto status = runner.check_status("fs", "create", vfs.create("/tmp/debug.txt", fs::NodeType::regular));
        !status.ok())
    {
        return status;
    }
...
Status test_ext4(TestPointRunner &runner)
{
    ...
    if (auto status = runner.check("ext4", "superblock",
