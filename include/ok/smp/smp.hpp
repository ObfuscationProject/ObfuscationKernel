...
private:
    [[nodiscard]] bool valid(CpuId cpu) const { return cpu < cpu_count_; }

    std::array<CpuInfo, max_cpus> cpus_ {};
    usize cpu_count_ {0};
};

template <typename T, usize Capacity = max_cpus>
class PerCpu final {
public:
    [[nodiscard]] T& get(CpuId cpu) { return values_[cpu]; }
    [[nodiscard]] const T& get(CpuId cpu) const { return values_[cpu]; }
