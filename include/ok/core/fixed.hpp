...
public:
    ...
    constexpr explicit FixedString(std::string_view value) { assign(value); }
...
public:
    ...
    [[nodiscard]] constexpr usize size() const { return size_; }
    [[nodiscard]] constexpr usize capacity() const { return Capacity; }
    [[nodiscard]] constexpr bool empty() const { return size_ == 0; }
...
constexpr void fill_span(std::span<T, N> span, T value)
{
    for (auto& item : span) {
        item = value;
    }
...
template <typename T>
constexpr void copy_span(std::span<T> destination, std::span<const T> source)
{
