...
public:
    ...
    [[nodiscard]] usize page_size() const { return page_size_; }
    [[nodiscard]] usize total_frames() const { return frame_used_.size(); }
...
public:
    [[nodiscard]] std::string_view name() const override { return "linear-address-space"; }
    Status map(uptr virtual_address, PhysicalFrame frame, usize flags) override;
    Status unmap(uptr virtual_address) override;
    [[nodiscard]] usize mapping_count() const { return mappings_.size(); }
...
public:
    Status initialize(std::span<const MemoryRegion> regions, usize page_size);
    [[nodiscard]] FrameAllocator& frames() { return frames_; }
    [[nodiscard]] const FrameAllocator& frames() const { return frames_; }
    [[nodiscard]] LinearAddressSpace& kernel_address_space() { return kernel_space_; }
